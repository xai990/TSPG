...
def plot_umap(
    ...
    # compute embedding of merged data
    reducer = umap.UMAP()
    embedding = reducer.fit_transform(x_merged)

    # separate embedded data back into original and perturbed
    x      = embedding[:len(x)]
    x_pert = embedding[len(x):]
    ...
    target = target
    plt.tight_layout()
    plt.savefig('%s/%s.umap.png' % (output_dir, classes[target]))
    plt.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--umap', help='plot u-map of samples', action='store_true')
    ...
    # plot u-map visualization if specified
    if args.umap:
